...
function LocalTime({ label, dark = false }: { label: string; dark?: boolean }) {
  ...
  return (
    <div className="text-center">
      <span className={`block ${dark ? "text-[#f4f2ec]/48" : "text-[#1e1e1e]/48"}`}>{label}</span>
      <time className={`mt-1.5 block font-medium tabular-nums tracking-[0.08em] sm:mt-2 ${dark ? "text-[#f4f2ec]/82" : "text-[#1e1e1e]/82"}`}>
        {localTime} {localZone}
      </time>
    </div>
  )
...
export default function FinalCtaContent({
  ...
  const mutedInk = isDark ? "text-[#f4f2ec]/48" : "text-[#1e1e1e]/48"
  const detailInk = isDark ? "text-[#f4f2ec]/82" : "text-[#1e1e1e]/82"
  const pillClassName = `group relative inline-flex h-13 w-full max-w-sm items-center justify-center overflow-hidden rounded-full border px-5 text-center text-[11px] font-medium transition-[border-color,color,transform] duration-500 ease-[cubic-bezier(.16,1,.3,1)] hover:-translate-y-0.5 focus-visible:outline-2 focus-visible:outline-offset-4 sm:h-16 sm:w-72 sm:px-7 sm:text-[13px] lg:h-18 ${
    isDark
      ? "border-[#f4f2ec]/30 text-[#f4f2ec]/86 hover:border-[#f4f2ec]/70 hover:text-[#1e1e1e] focus-visible:outline-[#f4f2ec]"
      : "border-[#1e1e1e]/30 text-[#1e1e1e]/86 hover:border-[#1e1e1e]/70 hover:text-[#f4f2ec] focus-visible:outline-[#1e1e1e]"
  }`
  const pillFill = isDark ? "bg-[#f4f2ec]" : "bg-[#1e1e1e]"
  ...
  return (
    <motion.section
      ...
      className={`relative flex min-h-svh w-full overflow-x-clip px-4 pb-24 pt-24 sm:overflow-hidden sm:px-8 sm:pb-24 sm:pt-16 lg:px-12 lg:pb-10 lg:pt-20 ${isDark ? "bg-[#1e1e1e] text-[#f4f2ec]" : "bg-[#f4f2ec] text-[#1e1e1e]"}`}
    >
      <div className="relative mx-auto flex w-full max-w-370 flex-1 flex-col justify-between gap-8 sm:gap-10">
        <div className="flex flex-1 flex-col items-center justify-center text-center sm:items-stretch sm:pb-10 sm:text-left lg:pb-14">
          ...
          <div className={`relative z-20 mt-14 w-full border-t sm:mt-[clamp(2.5rem,6vh,5.5rem)] ${isDark ? "border-[#f4f2ec]/28" : "border-[#1e1e1e]/28"}`}>
            <div
              ref={buttonAnchorRef}
              ...
              >
                <motion.div style={{ x, y }}>
                  <Link
                    ...
                    className={`group relative inline-flex aspect-square w-34 items-center justify-center overflow-hidden rounded-full bg-[#552f22] p-5 text-center font-display text-[11px] font-semibold uppercase leading-tight text-[#f4f2ec] transition-[transform,box-shadow,color] duration-700 hover:scale-[1.07] focus-visible:outline-2 focus-visible:outline-offset-4 sm:w-[clamp(112px,13vw,202px)] sm:text-[clamp(.68rem,.9vw,.88rem)] ${isDarkButton ? "hover:text-[#552f22] focus-visible:outline-[#f4f2ec]" : "focus-visible:outline-[#1e1e1e]"}`}
                  >
                    <span aria-hidden="true" className={`absolute inset-0 origin-bottom scale-y-0 rounded-full transition-transform duration-700 ease-[cubic-bezier(.16,1,.3,1)] group-hover:scale-y-100 ${isDarkButton ? "bg-[#f4f2ec]" : "bg-[#1e1e1e]"}`} />
        ...
        <footer className={`grid grid-cols-2 gap-x-4 gap-y-5 border-t pt-5 text-center text-[9px] uppercase tracking-[0.12em] sm:grid-cols-[1fr_1fr_1fr] sm:items-end sm:gap-6 sm:border-0 sm:pt-0 sm:text-left sm:text-[11px] lg:text-xs ${isDark ? "border-[#f4f2ec]/16" : "border-[#1e1e1e]/16"}`}>
